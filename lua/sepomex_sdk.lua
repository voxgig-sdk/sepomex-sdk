...
SepomexSDK._make_feature = _make_feature


function SepomexSDK.new(options)
  local self = setmetatable({}, SepomexSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function SepomexSDK:options_map()
...
function SepomexSDK:get_utility()
  return Utility.copy(self._utility)
end


function SepomexSDK:get_root_ctx()
  return self._rootctx
end


function SepomexSDK:prepare(fetchargs)
...
function SepomexSDK:direct(fetchargs)
...
-- by passing it.
function SepomexSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function SepomexSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "SepomexSDK: graphql: " .. msg
    res.graphql = errors
  end
...
-- Idiomatic facade: client:ZipCode():list() / client:ZipCode():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
...
function SepomexSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = SepomexSDK.new(sdkopts)
  sdk.mode = "test"
