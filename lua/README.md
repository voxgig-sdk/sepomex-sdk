...
The Lua SDK for the Sepomex API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:City(nil):list(nil, nil)
...
```lua
local result, err = client:City(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:Sepomex(nil):load(
  { id = "test01" }, nil
)
