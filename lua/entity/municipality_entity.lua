...
local MunicipalityEntity = {}
MunicipalityEntity.__index = MunicipalityEntity


function MunicipalityEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MunicipalityEntity)
  self._name = "municipality"
  self._client = client
...
function MunicipalityEntity:get_name()
  return self._name
end


function MunicipalityEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MunicipalityEntity.new(self._client, opts)
end
...
function MunicipalityEntity:mark_deleted()
  self._deleted = true
end


function MunicipalityEntity:deleted()
  return true == self._deleted
end


function MunicipalityEntity:data_set(args)
  if args ~= nil then
...
function MunicipalityEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MunicipalityEntity:match_set(args)
  if args ~= nil then
...
function MunicipalityEntity:match_get()
...
function MunicipalityEntity:stream(action, args, callopts)
...
---@param reqmatch MunicipalityListMatch
---@param ctrl? table
---@return Municipality[]
---@return string? err
function MunicipalityEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function MunicipalityEntity:_run_op(ctx, post_done)
  local utility = self._utility
