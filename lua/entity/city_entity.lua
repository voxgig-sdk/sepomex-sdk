...
local CityEntity = {}
CityEntity.__index = CityEntity


function CityEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CityEntity)
  self._name = "city"
  self._client = client
...
function CityEntity:get_name()
  return self._name
end


function CityEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CityEntity.new(self._client, opts)
end
...
function CityEntity:mark_deleted()
  self._deleted = true
end


function CityEntity:deleted()
  return true == self._deleted
end


function CityEntity:data_set(args)
  if args ~= nil then
...
function CityEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CityEntity:match_set(args)
  if args ~= nil then
...
function CityEntity:stream(action, args, callopts)
...
---@param reqmatch CityLoadMatch
---@param ctrl? table
---@return City
---@return string? err
function CityEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch CityListMatch
---@param ctrl? table
---@return City[]
---@return string? err
function CityEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function CityEntity:_run_op(ctx, post_done)
  local utility = self._utility
