...
local ZipCodeEntity = {}
ZipCodeEntity.__index = ZipCodeEntity


function ZipCodeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ZipCodeEntity)
  self._name = "zip_code"
  self._client = client
...
function ZipCodeEntity:get_name()
  return self._name
end


function ZipCodeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ZipCodeEntity.new(self._client, opts)
end
...
function ZipCodeEntity:mark_deleted()
  self._deleted = true
end


function ZipCodeEntity:deleted()
  return true == self._deleted
end


function ZipCodeEntity:data_set(args)
  if args ~= nil then
...
function ZipCodeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ZipCodeEntity:match_set(args)
  if args ~= nil then
...
function ZipCodeEntity:match_get()
...
function ZipCodeEntity:stream(action, args, callopts)
...
function ZipCodeEntity:_run_op(ctx, post_done)
  local utility = self._utility
