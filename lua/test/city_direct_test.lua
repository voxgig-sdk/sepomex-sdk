...
describe("CityDirect", function()
  it("should direct-list-city", function()
    local setup = city_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-city", setup.live and "live" or "unit")
  ...
  it("should direct-load-city", function()
    local setup = city_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-city", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function city_direct_setup(mockres)
