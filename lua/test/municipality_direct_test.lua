...
describe("MunicipalityDirect", function()
  it("should direct-list-municipality", function()
    local setup = municipality_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-municipality", setup.live and "live" or "unit")
  ...
  it("should direct-load-municipality", function()
    local setup = municipality_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-municipality", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function municipality_direct_setup(mockres)
