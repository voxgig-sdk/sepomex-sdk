...
MCP server exposing the Sepomex SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `sepomex_list` | `entity`, optional `query` map | First page of records as JSON |
| `sepomex_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `sepomex_list` and
`sepomex_load` tools become available in new sessions.
