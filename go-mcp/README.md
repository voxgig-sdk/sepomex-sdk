...
[MCP](https://modelcontextprotocol.io) server exposing the Sepomex SDK as
two agent tools — `sepomex_list` and `sepomex_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/sepomex-mcp)
make build

# 2. Provide credentials via the environment
export SEPOMEX_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user sepomex \
  -- /absolute/path/to/sepomex-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./sepomex-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// sepomex_list: first page of records
{ "entity": "city" }
{ "entity": "city", "query": { } }

// sepomex_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/sepomex-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user sepomex \
     -- "$PWD"/dist/*/sepomex-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `sepomex_list` and `sepomex_load` tools now appear
   in new sessions. Ask the agent to *"list city using sepomex"*
   and it calls `sepomex_list` with `{"entity":"city"}`.
...
```sh
export SEPOMEX_APIKEY=sk_live_xxx            # API key
export SEPOMEX_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `sepomex_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `sepomex_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `SEPOMEX_APIKEY` | API key sent with every request. |
| `SEPOMEX_BASE` | Optional override of the API base URL. |

### Entities

The 4 entities valid as the `entity` argument:

city | municipality | state | zip_code

### Smoke test via HTTP (raw JSON-RPC)

```sh
./sepomex-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"sepomex_load","arguments":{"entity":"city","query":{"id":1}}}}'
```
