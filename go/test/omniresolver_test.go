...
import (
	...
	omni "github.com/voxgig-sdk/sepomex-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.SepomexError); is {
				out := map[string]any{
					"name":    "SepomexError",
...
func omniCtx(arg any, client *sdk.SepomexSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.SepomexSDK); is {
			client = live
