...
func TestStateDirect(t *testing.T) {
	...
	t.Run("direct-load-state", func(t *testing.T) {
		setup := stateDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-state", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
func stateDirectSetup(mockres any) *stateDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"SEPOMEX_TEST_STATE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["SEPOMEX_TEST_STATE_ENTID"]; ok {
