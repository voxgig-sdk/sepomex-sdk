...
class FtCtrl extends SepomexControl
{
...
class FtClient
{
    ...
    private SepomexUtility $utility;

    public function __construct(SepomexUtility $utility, array $options)
    {
    ...
    public function get_utility(): SepomexUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (SepomexContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (SepomexContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = SepomexConfig::make_feature($name);
    ...
    private static function buildUrl(SepomexSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof SepomexError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new SepomexResponse($fetched);
        }
    ...
    private function fail(SepomexContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (SepomexContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
