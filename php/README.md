...
The PHP SDK for the Sepomex API. Provides an entity-oriented interface using PHP conventions.
...
```php
[$result, $err] = $client->City(null)->load(["id" => "example_id"], null);
if ($err) { throw new \Exception($err); }
print_r($result);
```
...
[$result, $err] = $client->Sepomex(null)->load(
    ["id" => "test01"], null
);
