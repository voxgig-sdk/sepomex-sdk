...
class SepomexSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = SepomexHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = SepomexHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!SepomexFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, SepomexFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return SepomexUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = SepomexHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = SepomexHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = SepomexHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new SepomexError($op . "_allow",
                "SepomexSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = SepomexHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new SepomexError("graphql_error",
                "SepomexSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Municipality($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_municipality === null) {
                $this->_municipality = new MunicipalityEntity($this, null);
            }
            return $this->_municipality;
        }
        return new MunicipalityEntity($this, $data);
    }
    ...
    // Canonical facade: $client->ZipCode()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->zip_code()
    // resolves here too.
    public function ZipCode($data = null)
    {
        require_once __DIR__ . '/entity/zip_code_entity.php';
        if ($data === null) {
            if ($this->_zip_code === null) {
                $this->_zip_code = new ZipCodeEntity($this, null);
            }
            return $this->_zip_code;
        }
        return new ZipCodeEntity($this, $data);
    }
