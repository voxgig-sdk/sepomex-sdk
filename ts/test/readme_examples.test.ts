...
//     (\`new SepomexSDK(...)\` / \`SepomexSDK.test(...)\` become
//     \`SepomexSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"city":{"test01":{"id":"test01"}},"municipality":{"test01":{"id":"test01"}},"state":{"test01":{"id":"test01"}},"zip_code":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new SepomexSDK(...)\` and any
// \`SepomexSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+SepomexSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/SepomexSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
