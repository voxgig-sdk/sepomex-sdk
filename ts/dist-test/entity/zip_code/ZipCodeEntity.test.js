...
(0, node_test_1.describe)('ZipCodeEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['list']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'zip_code.' + op, live))
                return;
        }
        ...
        let zip_code_ref01_data = Object.values(setup.data.existing.zip_code)[0];
        // LIST
        const zip_code_ref01_ent = client.ZipCode();
        const zip_code_ref01_match = {};
        const zip_code_ref01_list = (await zip_code_ref01_ent.list(zip_code_ref01_match)).map((e) => e.data());
    });
});
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/zip_code/ZipCodeTestData.json');
    ...
    let idmap = transform(['zip_code01', 'zip_code02', 'zip_code03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['SEPOMEX_TEST_ZIP_CODE_ENTID'];
...
//# sourceMappingURL=ZipCodeEntity.test.js.map