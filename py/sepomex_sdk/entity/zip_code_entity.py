...
from sepomex_sdk.sepomex_types import (
    ZipCode,
    ZipCodeListMatch,
)
...
class ZipCodeEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ZipCodeEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[ZipCode]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.ZipCode().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
