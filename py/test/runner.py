# Sepomex SDK test runner

from __future__ import annotations
import os
import json

from sepomex_sdk.utility.voxgig_struct import voxgig_struct as vs


class SepomexTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            SepomexTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = SepomexTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = SepomexTestRunner.getenv("SEPOMEX_TEST_LIVE")
        override = SepomexTestRunner.getenv("SEPOMEX_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = SepomexTestRunner.getenv(key)
        ...
        explain = SepomexTestRunner.getenv("SEPOMEX_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["SEPOMEX_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if SepomexTestRunner._test_control is not None:
            return SepomexTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                SepomexTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            SepomexTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = SepomexTestRunner.load_test_control()
...
def load_env_local():
    SepomexTestRunner.load_env_local()


def env_override(m):
    return SepomexTestRunner.env_override(m)


def entity_data(v):
    return SepomexTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return SepomexTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return SepomexTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return SepomexTestRunner.load_test_control()


def live_client_options():
    return SepomexTestRunner.live_client_options()


def live_delay_ms():
    return SepomexTestRunner.live_delay_ms()
