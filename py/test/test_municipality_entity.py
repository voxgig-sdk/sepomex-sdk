...
class TestMunicipalityEntity:

    def test_should_create_instance(self):
        testsdk = SepomexSDK.test(None, None)
        ent = testsdk.Municipality(None)
    ...
    def test_should_stream(self):
        ...
        base = SepomexSDK.test(seed, None)
        seen = list(base.Municipality(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Municipality(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _municipality_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "municipality." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set SEPOMEX_TEST_MUNICIPALITY_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        municipality_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.municipality")))
        municipality_ref01_data = None
        if len(municipality_ref01_data_raw) > 0:
        ...
        municipality_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(municipality_ref01_data_dt0_loaded))
...
def _municipality_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/municipality/MunicipalityTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("SEPOMEX_TEST_MUNICIPALITY_ENTID"))
