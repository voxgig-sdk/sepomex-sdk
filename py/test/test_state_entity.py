    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set SEPOMEX_TEST_STATE_ENTID JSON to run live")
        client = setup["client"]
        ...
        state_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(state_ref01_data_dt0_loaded))
        assert state_ref01_data_dt0_load_result is not None
        assert state_ref01_data_dt0_load_result["id"] == state_ref01_data["id"]
...
def _state_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("SEPOMEX_TEST_STATE_ENTID"))
