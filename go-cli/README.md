...
boru-driven command-line client **and** interactive REPL for the Sepomex
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/sepomex-cli)
make build
...
./sepomex-cli list city
./sepomex-cli load 1 city            # {id:1} shorthand
./sepomex-cli load '{id:1}' city       # explicit match map
./sepomex-cli list municipality

# 5. Override the API base URL for a single call
SEPOMEX_BASE=https://api.example.com ./sepomex-cli list city
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/sepomex-cli
   ```
...
```sh
./sepomex-cli load 1 city          # scalar shorthand for {id:1}
./sepomex-cli load '{id:1}' city     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `sepomex>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `city`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/sepomex-cli`. |
...
(`./sepomex-cli <expr>`) and interactively (the REPL), and expressions compose the
