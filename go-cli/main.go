// Command Sepomex-cli is a CLI + REPL for the Sepomex SDK,
...
const prompt = "sepomex"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "city municipality state zip_code"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      SEPOMEX_APIKEY (api key), SEPOMEX_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: SEPOMEX_APIKEY carries the API key and
	// SEPOMEX_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("SEPOMEX_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("SEPOMEX_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewSepomexSDK(opts)
