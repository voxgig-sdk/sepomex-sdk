...
class MunicipalityEntity
  ...
  def make
    opts = @_entopts.dup
    MunicipalityEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.Municipality.load works with no args).
  ...
  #
  # @param reqmatch [MunicipalityListMatch, Hash, nil] match filter (any subset of
  #   Municipality fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
