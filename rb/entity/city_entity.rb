...
class CityEntity
  ...
  def make
    opts = @_entopts.dup
    CityEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = SepomexHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [City, Hash] the current City data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = SepomexHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of City fields)
  def match_get
  ...
  # Load a single City.
  #
  # @param reqmatch [CityLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.City.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [City, Hash] the loaded City; raises SepomexError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = SepomexHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [CityListMatch, Hash, nil] match filter (any subset of
  #   City fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<City>, Array] the matching City items; raises SepomexError on failure
