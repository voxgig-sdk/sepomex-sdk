...
class ZipCodeDirectTest < Minitest::Test
  def test_direct_list_zip_code
    setup = zip_code_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-zip_code", setup[:live] ? "live" : "unit")
    if _should_skip
...
def zip_code_direct_setup(mockres)
