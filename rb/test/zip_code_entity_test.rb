...
class ZipCodeEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.ZipCode(nil).stream("list", nil, nil).to_a
    assert_equal 3, seen.length

    # Inbound: streaming active -> yields each item from the feature.
    cfg = SepomexConfig.shared_config
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      sdk = SepomexSDK.test(seed, { "feature" => { "streaming" => { "active" => true } } })
      got = []
      sdk.ZipCode(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = zip_code_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "zip_code." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set SEPOMEX_TEST_ZIP_CODE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    zip_code_ref01_data = nil
    if zip_code_ref01_data_raw.length > 0
...
def zip_code_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "zip_code", "ZipCodeTestData.json")
  ...
  entid_env_raw = ENV["SEPOMEX_TEST_ZIP_CODE_ENTID"]
